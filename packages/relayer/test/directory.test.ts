...
describe('the X binding', () => {
  it('binds with the same proof a claim needs, and the list wears the attested handle', async () => {
    const { directory } = fresh()
    const bound = await directory.bindX({ ...claimBody('nightowl'), xHandle: 'NightOwl', xId: '12345' })
    expect(bound).toEqual({ ok: true })
    expect(directory.list()).toEqual([
      { name: 'nightowl', address: ADDRESS, hasAvatar: false, xHandle: 'NightOwl' },
    ])
  })

  it('refuses a bind whose signature does not prove the address — OAuth is not the proof', async () => {
    const { directory } = fresh()
    const forged = await directory.bindX({
      ...claimBody('nightowl', ADDRESS, OTHER_KEY),
      xHandle: 'NightOwl',
      xId: '12345',
    })
  ...
  it('refuses malformed x fields before any chain read', async () => {
    const { directory, readPublicKey } = fresh()
    expect(await directory.bindX({ ...claimBody('sam'), xHandle: 'way too long for an x handle', xId: '1' })).toMatchObject({ ok: false, status: 400 })
    expect(await directory.bindX({ ...claimBody('sam'), xHandle: 'sam', xId: 'not-numeric' })).toMatchObject({ ok: false, status: 400 })
    expect(readPublicKey).not.toHaveBeenCalled()
  })

  it('a plain re-claim keeps the X stamp — renaming does not un-verify', async () => {
    const { directory } = fresh()
    await directory.bindX({ ...claimBody('nightowl'), xHandle: 'NightOwl', xId: '12345' })
    // The same holder re-claims the same name without X in the loop at all.
    expect(await directory.claim(claimBody('nightowl'))).toEqual({ ok: true })
    expect(directory.list()[0]).toMatchObject({ xHandle: 'NightOwl' })
  })

  it('a claim NEVER creates a stamp — only bindX can', async () => {
    const { directory } = fresh()
    await directory.claim({ ...claimBody('sam'), xHandle: 'Sam', xId: '1' } as never)
    expect(directory.list()[0]).not.toHaveProperty('xHandle')
  })
})
