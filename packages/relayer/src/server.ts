...
// would mean teaching `handle()` — the one function every security gate in this file lives in —
// to parse a URL, which it deliberately never does. It travels in the JSON body instead, which
// costs the client nothing and keeps every gate matching an exact string.
//
...
// with no faucet ledger: this deployment does not hand out STRK at all, so there is nothing for
// a client to retry.
//
...
async function handle(req: IncomingMessage, res: ServerResponse, opts: RelayerServerOptions) {
  ...
  // A relayer with no hub answers 404 on both room routes — NOT FOUND rather than refused, which
  // is the honest answer: this deployment does not carry chat at all, so there is nothing for a
  // client to retry.
  ...
  // No feed, no route — the same rule again: a deployment with nothing to stream answers 404,
  ...
  // BEHIND ALL FOUR GATES ABOVE, in the same order, for the same reasons. Same door, same lock,
  // and the directory is the one record that WANTS to be read — the gates
...
export interface RelayerServerOptions {
   ...
   *
   * It also uses `tryClaim`: the claim set holds `drip:<felt>` keys, one per funded address, so
   * "once per address, ever" survives a restart. The mechanism is a one-shot atomic burn, which
   * is exactly what the drip needs.
   *
