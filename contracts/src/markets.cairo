...
/// hand-written ABI.
///
/// `collateral` is the whole pot — seed plus every stake taken since. The design sketch carried
/// `seed` and `open_cash` as separate fields; that is the same number split in two, and the split
/// is recoverable (`open_cash == collateral - seed`), so only the total is stored. Conservation is
/// stated over `collateral`, and a quantity that an invariant is stated over should have exactly
/// one representation.
...
pub mod Markets {
    ...
    /// sits under that and also bounds the duplicate-token scan in `first_occurrence_total`.
    ...
    #[constructor]
    fn constructor(ref self: ContractState, pool: ContractAddress, pragma: ContractAddress) {
    ...
    impl MarketsImpl of super::IMarkets<ContractState> {
        ...
        fn quote_bet(self: @ContractState, market_id: u64, side: u8, amount: u128) -> u128 {
            ...
            tickets_for(market.up, market.down, market.k, side, amount)
        }
    ...
    impl InternalImpl of InternalTrait {
        ...
        fn op_bet(ref self: ContractState, payload: Span<felt252>) -> Span<OpenNoteDeposit> {
            ...
            while i != n {
                ...
                let tickets = tickets_for(market.up, market.down, market.k, side, amount);
                // A stake so small it rounds to nothing would take the money and issue no claim.
                assert(tickets != 0, 'ZERO_TICKETS');

                if side == SIDE_UP {
                    market.up = market.up + amount - tickets;
                    market.down = market.down + amount;
                } else {
                    market.down = market.down + amount - tickets;
                    market.up = market.up + amount;
                }
    ...
    ///
    /// `stride` is the felts per entry, so the length check is exact rather than a lower bound —
    /// a payload with a trailing felt is a client bug, and silently ignoring it settles the wrong
    /// batch.
    fn read_batch_len(payload: Span<felt252>, stride: u32) -> u32 {
    ...
    ///
    /// Callers read 0 as "skip", which is only safe because no individual amount in a batch is
    /// ever zero — both call sites assert that before they get here.
    ///
    /// Quadratic, deliberately. `MAX_BATCH` is 64 and a real ladder is three, so a scan is
    /// cheaper in gas and far cheaper in review effort than carrying a dictionary through a loop.
    fn first_occurrence_total(
    ...
    /// Approve the pool ONCE per distinct token, for that token's whole batch total.
    ///
    /// This is the difference between a batch settling and a batch reverting. StarkWare's own
    /// shadow_account_anonymizer approves inside its deposit loop; because `approve` overwrites
    /// rather than accumulates, the last approval is the only one standing and every earlier pull
    /// in a same-token batch finds too little allowance. Summing first is why claiming a
    /// three-strike ladder fits in one transaction here and does not there.
    fn approve_batch_totals(
