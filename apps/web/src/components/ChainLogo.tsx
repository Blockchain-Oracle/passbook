//
// The chain marks. Inline SVG components, one per bridge destination — never fetched, never a
// letter-fallback disc. A chain badge that renders "POL" in a coloured circle reads as a missing
// image; these are the marks people actually recognise.
//
// Provenance: Polygon and Solana are the official svgl.app vectors verbatim (matic.svg, sol.svg).
// Ethereum is the classic two-facet diamond on Uniswap's network colour. Base, Optimism and
// Arbitrum are drawn here from their published brand geometry (circle + chord bar; OP letters;
// the mountain mark) on their official colours — Uniswap's own network-colour tokens
// (`--color-network-*` in their theme.css) are the colour source of truth.
//
// Gradient/clip ids are namespaced `pbchain-*`; duplicate ids across instances resolve to
// identical defs, so multiple badges on one page paint correctly.
//
...
function BaseMark({ size }: MarkProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 36 36" aria-hidden="true">
      ...
      {/* The official mark: the bar enters from the circle's left edge and stops short of centre-right. */}
...
function ArbitrumMark({ size }: MarkProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 36 36" aria-hidden="true">
      ...
      {/* The mountain mark, two ascents on the brand blue with a white leading edge. */}
...
export interface ChainLogoProps {
  /** The bridge destination key. Unknown keys render nothing — the caller supplies its own fallback. */
  chainKey: string
  size?: number
  className?: string
}
...
export function ChainLogo({ chainKey, size = 40, className }: ChainLogoProps) {
  if (!isKnownChain(chainKey)) return null
  const Mark = MARKS[chainKey]
  return (
